...
export function ExpenseForm({ initialExpense, onSubmit, onCancel }: ExpenseFormProps) {
  ...
  return <><form className="max-w-2xl space-y-6 rounded-2xl border border-white/8 bg-[#151818] p-5 shadow-[0_24px_80px_rgb(0_0_0/0.2)] sm:p-7" onSubmit={handleSubmit}>
    ...
    <label className="field-label">Description <span className="text-[#64736b]">{description.length}/120</span><input className="field-input" maxLength={120} onChange={(event) => update('description', event.target.value)} placeholder="What did you spend on?" type="text" value={description} /></label>
